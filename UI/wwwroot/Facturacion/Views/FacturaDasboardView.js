//@ts-check

import { StylesControlsV2, StylesControlsV3, StyleScrolls } from "../../WDevCore/StyleModules/WStyleComponents.js";
import { WCarousel } from "../../WDevCore/WComponents/UIComponents/WCarousel.js";
import { WAppNavigator } from "../../WDevCore/WComponents/WAppNavigator.js";
import { ColumChart } from "../../WDevCore/WComponents/WChartJSComponents.js";
import { DateTime } from "../../WDevCore/WModules/Types/DateTime.js";
import { WArrayF } from "../../WDevCore/WModules/WArrayF.js";
import { ComponentsManager, ConvertToMoneyString, html, WRender } from "../../WDevCore/WModules/WComponentsTools.js";
import { css } from "../../WDevCore/WModules/WStyledRender.js";
import { TransactionReport } from "../FrontModel/TransactionReport.js";

/**
 * @typedef {Object} ComponentConfig
 * * @property {Object} [propierty]
 */
class FacturacionDasboardView extends HTMLElement {
    /**
     * 
     * @param {ComponentConfig} [Config] 
     */
    constructor(Config) {
        super();
        this.Config = Config
        /**
         * @type {String?}
         */
        this.ComponentsTitle = null;
        this.OptionContainer = WRender.Create({ className: "OptionContainer" });
        this.TabContainer = WRender.Create({ className: "TabContainer", id: 'TabContainer' });//usar si es neecsario
        this.Manager = new ComponentsManager({ MainContainer: this.TabContainer, SPAManage: false }); //usar si es neecsario

        this.append(this.CustomStyle);
        this.append(
            StylesControlsV2.cloneNode(true),
            StyleScrolls.cloneNode(true),
            StylesControlsV3.cloneNode(true)

        );

    }
    connectedCallback() {
        this.AppNavigator = new WAppNavigator({
            NavStyle: "tab",
            NavTitle: this.ComponentsTitle ?? "",
            Inicialize: true,
            Elements: this.SetOption(),
            TabContainer: this.TabContainer
        })
        this.append(this.AppNavigator, this.TabContainer)
    }

    SetOption() {
        return [
            {
                name: "Inicio",
                action: async () => {
                    this.GetFacturasHoy = await new TransactionReport().GetFacturasHoy();
                    this.GetFacturasSemana = await new TransactionReport().GetFacturasSemana();
                    this.GetFacturasMes = await new TransactionReport().GetFacturasMes();
                    this.GetFacturasAnio = await new TransactionReport().GetFacturasAnio();
                    return this.MainComponent()
                }
            }
        ]
    }
    MainComponent() {
        const carroucel = new WCarousel([
            this.Card(this.GetFacturasHoy, `Informe ${new DateTime(new Date()).toDateFormatEs()}`),
            this.Card(this.GetFacturasSemana, `Informe del ${new DateTime(new Date()).getDateWeakRange()}`),
            this.Card(this.GetFacturasMes, `Informe ${new DateTime(new Date()).getMonthFormatEs()}`),
            this.Card(this.GetFacturasAnio, `Informe ${new DateTime(new Date()).getFullYear()}`)
        ])
        return html`<div class="component">${carroucel}</div>`;
    }
    /**
    * @param {Array<TransactionReport>} Dataset
    * @param {string} title
    * @returns {HTMLElement}
    */
    Card(Dataset, title) {
        console.log(Dataset);

        const columnDebito = new ColumChart({
            Dataset: Dataset.filter(d => d.Debito != 0),
            AttNameEval: "Tipo_movimiento",
            EvalValue: "Debito", // suma el valor del value para obtener la sumatoria completa del array
            groupParams: ["Nombre"],
        });
        const columnCredito = new ColumChart({
            Dataset: Dataset.filter(d => d.Credito != 0),
            AttNameEval: "Tipo_movimiento",
            EvalValue: "Credito", // suma el valor del value para obtener la sumatoria completa del array
            groupParams: ["Nombre"],
        });
        const columnDebitoUSD = new ColumChart({
            Dataset: Dataset.filter(d => d.Debito_dolares != 0),
            AttNameEval: "Tipo_movimiento",
            EvalValue: "Debito_dolares", // suma el valor del value para obtener la sumatoria completa del array
            groupParams: ["Nombre"],
        });
        const columnCreditoUSD = new ColumChart({
            Dataset: Dataset.filter(d => d.Credito_dolares != 0),
            AttNameEval: "Tipo_movimiento",
            EvalValue: "Credito_dolares", // suma el valor del value para obtener la sumatoria completa del array
            groupParams: ["Nombre"],
        });


        const totalCredito = WArrayF.SumValAtt(Dataset, "Credito");
        const totalDebito = WArrayF.SumValAtt(Dataset, "Debito");

        const totalCreditoUSD = WArrayF.SumValAtt(Dataset, "Credito_dolares");
        const totalDebitoUSD = WArrayF.SumValAtt(Dataset, "Debito_dolares");

        // @ts-ignore
        const data = Object.groupBy(Dataset, p => p.Nombre)

        const dataProcess = Object.keys(data).map(key => {
            return {
                name: key,
                debito: WArrayF.SumValAtt(data[key], "Debito"),
                debitoUSD: WArrayF.SumValAtt(data[key], "Debito_dolares"),
                credito: WArrayF.SumValAtt(data[key], "Credito"),
                creditoUSD: WArrayF.SumValAtt(data[key], "Credito_dolares")
            }
        });

        return html`<div class="space-y-5">
            <h3>${title}</h3>
        <div class="grid grid-cols-12 gap-5">
            <div class="lg:col-span-8 space-y-5 col-span-12">
                <div class="grid grid-cols-4 gap-5">
                    <div class="card   text-center py-3 !bg-indigo-500">
                        <main class="card-body px-5 py-4">
                            <div
                                class="h-16 w-16 mx-auto rounded-full text-indigo-500  bg-indigo-50 mb-4 text-4xl flex  items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                    aria-hidden="true" role="img" class="iconify iconify--ph" width="1em" height="1em"
                                    viewBox="0 0 256 256">
                                    <path fill="currentColor"
                                        d="M128 88a40 40 0 1 0 40 40a40 40 0 0 0-40-40m0 64a24 24 0 1 1 24-24a24 24 0 0 1-24 24m112-96H16a8 8 0 0 0-8 8v128a8 8 0 0 0 8 8h224a8 8 0 0 0 8-8V64a8 8 0 0 0-8-8m-46.35 128H62.35A56.78 56.78 0 0 0 24 145.65v-35.3A56.78 56.78 0 0 0 62.35 72h131.3A56.78 56.78 0 0 0 232 110.35v35.3A56.78 56.78 0 0 0 193.65 184M232 93.37A40.8 40.8 0 0 1 210.63 72H232ZM45.37 72A40.8 40.8 0 0 1 24 93.37V72ZM24 162.63A40.8 40.8 0 0 1 45.37 184H24ZM210.63 184A40.8 40.8 0 0 1 232 162.63V184Z">
                                    </path>
                                </svg>
                            </div>
                            <div class=" font-bold  text-lg mb-[2px] text-white ">C$ ${ConvertToMoneyString(totalCredito)}</div>
                            <div class=" text-sm text-gray-200"> Ingreso NIO</div>
                        </main>
                    </div>
                    <div class="card text-center py-3 !bg-fuchsia-500">
                        <main class="card-body px-5 py-4">
                            <div
                                class="h-16 w-16 mx-auto rounded-full text-fuchsia-500 bg-fuchsia-50 mb-4 text-4xl flex  items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                    aria-hidden="true" role="img" class="iconify iconify--fluent" width="1em"
                                    height="1em" viewBox="0 0 24 24">
                                    <path fill="currentColor"
                                        d="M16.749 2h4.554l.1.014l.099.028l.06.026q.12.052.219.15l.04.044l.044.057l.054.09l.039.09l.019.064l.014.064l.009.095v4.532a.75.75 0 0 1-1.493.102l-.007-.102V4.559l-6.44 6.44a.75.75 0 0 1-.976.073L13 11L9.97 8.09l-5.69 5.689a.75.75 0 0 1-1.133-.977l.073-.084l6.22-6.22a.75.75 0 0 1 .976-.072l.084.072l3.03 2.91L19.438 3.5h-2.69a.75.75 0 0 1-.742-.648l-.007-.102a.75.75 0 0 1 .648-.743zM3.75 17a.75.75 0 0 1 .75.75v3.5a.75.75 0 0 1-1.5 0v-3.5a.75.75 0 0 1 .75-.75m5.75-3.25a.75.75 0 0 0-1.5 0v7.5a.75.75 0 0 0 1.5 0zM13.75 15a.75.75 0 0 1 .75.75v5.5a.75.75 0 0 1-1.5 0v-5.5a.75.75 0 0 1 .75-.75m5.75-4.25a.75.75 0 0 0-1.5 0v10.5a.75.75 0 0 0 1.5 0z">
                                    </path>
                                </svg>
                            </div>
                            <div class=" font-bold  text-lg mb-[2px]  text-white">C$ ${ConvertToMoneyString(totalDebito)}</div>
                            <div class=" text-sm text-gray-200"> Egreso NIO</div>
                        </main>
                    </div>
                    <div class="card text-center py-3 !bg-yellow-500">
                        <main class="card-body px-5 py-4">
                            <div
                                class="h-16 w-16 mx-auto rounded-full text-yellow-500 bg-yellow-50 mb-4 text-4xl flex  items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                    aria-hidden="true" role="img" class="iconify iconify--carbon" width="1em"
                                    height="1em" viewBox="0 0 32 32">
                                    <path fill="currentColor"
                                        d="M23 23v3H8.5a4.5 4.5 0 0 1 0-9H9v-2h-.5a6.5 6.5 0 0 0 0 13H23v3h8v-8Zm6 6h-4v-4h4Z">
                                    </path>
                                    <path fill="currentColor"
                                        d="M21 22h-2v-3h-6v3h-2v-3a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2zm-5-6a3 3 0 1 1 3-3a3.003 3.003 0 0 1-3 3m0-4a1 1 0 1 0 1 1a1 1 0 0 0-1-1">
                                    </path>
                                    <path fill="currentColor"
                                        d="M23.5 4H9V1H1v8h8V6h14.5a4.5 4.5 0 0 1 0 9H23v2h.5a6.5 6.5 0 0 0 0-13M7 7H3V3h4Z">
                                    </path>
                                </svg>
                            </div>
                            <div class=" font-bold  text-lg mb-[2px] text-white">$ ${ConvertToMoneyString(totalCreditoUSD)}</div>
                            <div class=" text-sm text-gray-200">Ingreso USD</div>
                        </main>
                    </div>
                    <div class="card text-center py-3 !bg-red-500">
                        <main class="card-body px-5 py-4">
                            <div
                                class="h-16 w-16 mx-auto rounded-full text-red-500 bg-red-50 mb-4 text-4xl flex  items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                    aria-hidden="true" role="img" class="iconify iconify--emojione-monotone" width="1em"
                                    height="1em" viewBox="0 0 64 64">
                                    <ellipse cx="10.97" cy="52.578" fill="currentColor" rx="1.742" ry="1.188"></ellipse>
                                    <path fill="currentColor" 
                                        d="m62 46.669l-.654-.384c-.05-.029-5.047-2.994-6.289-6.592l-.277-.797l-.512.669c-.016.021-.726.899-2.713 1.929c-1.471-8.373-6.475-18.369-13.051-22.631c1.88.152 3.766-.004 5.586-.666c1.096-.398.623-2.184-.483-1.78c-1.492.543-2.987.691-4.479.615c3.663-3.357 6.539-8.502 6.539-12.264c0-.904 0-2.587-1.34-2.587c-.538 0-.973.336-1.574.8c-.922.711-2.314 1.787-4.359 1.787c-.779 0-2.205-.822-3.351-1.482C33.658 2.489 32.773 2 32.029 2c-1.232 0-3.043.996-4.641 1.875c-.65.357-1.539.847-1.723.894c-2.316 0-3.661-1.068-4.553-1.775c-.52-.414-.897-.714-1.4-.714c-1.318 0-1.318 1.581-1.318 3.412c0 3.534 2.692 8.199 6.217 11.334c-2.221.159-4.411.83-5.95 2.321c-.85.822.438 2.126 1.285 1.306c1.279-1.24 2.956-1.771 4.75-1.874q.076.274.189.54C17.369 24.72 12.03 37.545 12.03 46.203c0 .854.066 1.627.168 2.355c-1.938-.861-2.645-1.616-2.659-1.633l-.516-.595l-.254.75c-1.123 3.304-6.005 6.17-6.054 6.199L2 53.694l.727.393c.255.138.508.259.763.39l-.46.319l.243 3.04L22.326 62l7.224-4.621q1.093.006 2.218.006L40.212 62l19.089-6v-3.209l-.505-.303q.378-.24.755-.491l.669-.447l-.147-.072l.851-.233l.144-3.336l-.492-.349q.393-.235.785-.483zM20.213 5.692c0-.444.006-.79.015-1.059c.153.12.331.251.521.386c.583 1.396 1.686 3.406 3.556 4.672c2.272 1.54 1.97-.23 3.182-1.846c1.212-1.615 2.032-2.461 4.062 1.693c.885 1.811 2.36-3.25 3.762-3.989c.371.202.741.39 1.104.55c2.514 4.088 3.679 2.813 5.912-.585c.614-.36 1.118-.746 1.519-1.055q.003.139.004.311c0 3.21-2.623 7.814-5.835 10.817a3.6 3.6 0 0 0-.642-1.053c-1.297-1.469-3.324.16-4.285 1.229c-1.705-.548-4.483-2.057-6.293-1.193a4.6 4.6 0 0 0-1.226.831c-2.96-2.698-5.356-6.593-5.356-9.709m16.204 10.974a27 27 0 0 1-1.337-.32c.558-.481 1.07-.649 1.285.066c.023.078.037.167.052.254m-7.6.621q-.496-.096-1.021-.165a14 14 0 0 1-.779-.525c.914-.613 2.49-.287 3.996.224c-.743.075-1.476.237-2.196.466m.454 2.64c-.56.14-1.17.084-1.697-.126c.214-.113.43-.211.646-.31c.35.169.7.31 1.051.436M10.774 56.379a39.3 39.3 0 0 1-6.912-2.733c1.357-.892 4.313-3.049 5.515-5.641c.824.647 2.726 1.827 6.43 2.733c-.063.102-.14.211-.214.318c-3.162-.828-4.96-1.875-4.989-1.893l-.436-.26l-.209.467c-.275.612-.92.704-1.279.704c-.1 0-.167-.007-.176-.008l-.357-.052l-.13.342c-.266.698-1.358 1.61-1.758 1.903l-.668.49l.764.313c.284.116.361.223.365.251c.01.069-.111.251-.24.356l-.582.469l.677.313c1.387.64 2.958 1.17 4.529 1.607q-.186.183-.33.321m8.064-3.726c1.398.196 2.947.32 4.639.32h.001q.527 0 1.073-.017a1 1 0 0 0 .103.213q.133.203.361.334l-2.552 2.181l-.146-.003c-1.137 0-1.985.418-2.392 1.162a69 69 0 0 1-4.602-.747a51 51 0 0 0 3.515-3.443M7.489 53.85c.1-.189.165-.418.13-.668a1 1 0 0 0-.307-.59c.449-.385 1.068-.979 1.408-1.594c.773-.011 1.411-.304 1.813-.822c.677.354 2.202 1.069 4.455 1.689c-.921 1.155-2.162 2.465-3.127 3.439c-1.504-.398-3.019-.878-4.372-1.454m14.251 4.209l-.205.001c-.861 0-3.714-.07-7.483-.855c.132-.112.269-.227.396-.337c3.058.639 5.524.913 5.709.934l.366.039l.113-.355c.247-.766 1.158-.879 1.68-.879c.143 0 .238.009.249.01l.194.021l4.433-3.788l-1.185-.039c-.396-.014-.563-.103-.604-.164l.251-.657l-.694.029q-.759.033-1.483.033h-.001c-1.396 0-2.691-.092-3.888-.237l.278-.328c2.416.323 5.309.522 8.802.497c-.834 1.17-2.914 3.657-6.928 6.075m10.024-4.345v1.825q-.853 0-1.665-.006l-.163-2.054l-1.354.11c.912-1.078 1.301-1.797 1.336-1.862l.367-.694l-.775.014c-7.418.136-12.256-.748-15.31-1.73c-.223-.902-.352-1.922-.352-3.113c0-8.123 5.191-20.401 12.083-25.377c1.221 1.181 3.073 1.698 4.833.549c.386-.252.722-.582 1.037-.933c.075.002.152.019.228.019c.252 0 .508-.021.763-.053c.323.129.639.259.932.388c1.403.618 2.601.234 3.419-.618c6.313 3.82 11.371 13.925 12.702 22.093c-2.886 1.154-7.464 2.28-14.49 2.478c1.386-.792 2.426-2.124 2.052-3.874c-.393-1.85-1.939-2.37-3.54-2.455q-.007-1.302.002-2.605c.491.217.962.467 1.39.734c.756.475 1.44-.756.689-1.231a10.3 10.3 0 0 0-2.072-.999q.015-1.508.015-3.014c.001-.92-1.366-.92-1.366 0q-.001 1.325-.014 2.646a7.1 7.1 0 0 0-2.202-.037a27 27 0 0 0-.432-1.97c-.226-.886-1.546-.511-1.317.378c.168.639.305 1.275.418 1.92c-.058.022-.121.033-.178.059c-1.852.733-2.982 2.602-1.627 4.345c.631.812 1.456 1.14 2.359 1.249c.049 1.203.08 2.407.115 3.609c-.605-.393-1.156-.897-1.655-1.43c-.618-.652-1.583.355-.966 1.006c.839.893 1.72 1.568 2.675 2c.043 1.027.104 2.057.204 3.079c.088.907 1.456.912 1.367 0a54 54 0 0 1-.187-2.657c.52.09 1.066.113 1.646.058c.065.946.141 1.892.249 2.838c.062.542.58.755.962.645l-.085 1.964c-.602.054-1.216.104-1.856.146l-.762.05l.406.656c.041.066.502.795 1.531 1.856h-1.412zm24.888-6.582c-1.257.591-2.847 1.133-4.467 1.606c-.996-1.029-2.27-2.402-3.221-3.62c2.119-.642 3.305-1.304 3.84-1.661c.423.373.983.567 1.646.566c.22 0 .415-.021.565-.045c.48.622 1.254 1.34 1.738 1.769q-.344.353-.331.776c.008.24.104.444.23.609m-7.155 3.286q.307.27.63.55c-4.054 1.075-7.071 1.192-7.728 1.203c-4.198-2.561-6.374-5.271-7.213-6.491c3.513-.088 6.417-.406 8.839-.844a24 24 0 0 0 .299.36a53 53 0 0 1-5.877.583l-.807.034l.444.679c-.001.001-.091.169-.687.285l-.969.189l4.764 3.857l.222-.06c.005-.001.425-.114.933-.114c.733 0 1.234.223 1.486.659l.16.276l.311-.055a87 87 0 0 0 5.193-1.111M35.4 47.479l-1.138-.062c-.053-.568-.106-1.136-.146-1.704c.2.332.61.956 1.284 1.766m-1.408-3.661a105 105 0 0 1-.119-4.041q.248.01.48.043c1.768.268 2.23 2.234.85 3.34c-.36.285-.775.494-1.211.658m10.439 6.752c-.448-.553-1.161-.843-2.083-.843c-.385 0-.724.052-.938.094l-2.995-2.427q.372-.214.509-.546a1 1 0 0 0 .052-.171c2.374-.13 4.374-.367 6.065-.657c.82.915 2.025 2.177 3.623 3.644c-1.963.467-3.6.787-4.233.906M32.505 39.813c.019 1.449.055 2.902.132 4.35c-.129.02-.256.045-.383.058c-.43.049-.838.013-1.226-.085c-.049-1.4-.081-2.803-.134-4.204c.538-.025 1.083-.078 1.611-.119m-1.68-1.265a44 44 0 0 0-.31-3.286a5.3 5.3 0 0 1 1.987.101a285 285 0 0 0-.005 3.067c-.439.024-.856.066-1.223.099a6 6 0 0 1-.449.019m-1.372-.175c-.085-.023-.169-.041-.256-.07a2.7 2.7 0 0 1-1.057-.667c-.854-.797.318-1.579 1.057-1.95c.116.89.196 1.788.256 2.687M40.49 57.839c-4.341-2.218-6.67-4.683-7.595-5.825c.665-.05 1.294-.11 1.917-.173l7.118 4.62l15.61-4.288c-8.05 4.852-16.003 5.591-17.05 5.666m12.892-7.89l-.462-.463c1.776-.538 3.494-1.162 4.775-1.848l.779-.419l-.79-.402c-.177-.091-.35-.258-.353-.321c.002-.024.072-.152.375-.326l.578-.332l-.514-.428c-.016-.013-1.619-1.344-2.182-2.201l-.186-.28l-.319.093c-.003 0-.277.079-.633.079c-.55.001-.962-.179-1.226-.533l-.292-.387l-.366.315c-.019.016-1.152.945-4.191 1.823a10 10 0 0 1-.289-.44c3.685-1.117 5.568-2.469 6.384-3.209c1.31 2.834 4.429 5.117 5.783 6.012a38 38 0 0 1-6.871 3.267">
                                    </path>
                                    <path fill="currentColor"
                                        d="M44.38 47.117c-.997-.271-2.122-.086-2.516.409c-.391.497.103 1.123 1.099 1.396c.998.271 2.122.085 2.516-.411c.391-.497-.101-1.121-1.099-1.394">
                                    </path>
                                </svg>
                            </div>
                            <div class=" font-bold  text-lg mb-[2px] text-white">$ ${ConvertToMoneyString(totalDebitoUSD)}</div>
                            <div class=" text-sm text-gray-200"> Egreso USD</div>
                        </main>
                    </div>
                </div>
                <div class="grid-cols-12 grid gap-5">
                    
                    <div class="lg:col-span-6 col-span-12">
                        <div class="card custom-class">
                            <header class="card-header  custom-header-class">
                                <div>
                                    <div class="card-title custom-class">Egresos NIO por transacción</div>
                                </div>
                            </header>
                            <main class="card-body px-5 py-4">
                                ${columnCredito}
                            </main>
                        </div>
                    </div>                
                    
                    <div class="lg:col-span-6 col-span-12">
                        <div class="card custom-class">
                            <header class="card-header  custom-header-class">
                                <div>
                                    <div class="card-title custom-class">Ingresos NIO por transacción</div>
                                </div>
                            </header>
                            <main class="card-body px-5 py-4">
                                ${columnDebito}
                            </main>
                        </div>
                    </div>
                </div>
            </div>
            <div class="lg:col-span-4 col-span-12">
                <div class="card  col-span-4">
                    <header class="card-header no-border custom-header-class">                        
                        <div class="card-title custom-class">Customer Review</div>
                    </header>
                    <main class="card-body px-5 py-4">
                        ${dataProcess.map(process => html`<div class=" space-y-4 mt-10">
                            <div class="flex flex-col items-start  space-x-4 rtl:space-x-reverse">
                                <div class="shrink-0 text-sm min-w-[70px]">${process.name}</div>
                                <hr>
                                <div class="grow">
                                    <div class="relative">
                                        <div
                                            class="w-full  overflow-hidden bg-opacity-10 progress  bg-green-600/25 rounded-full">
                                            <div class="flex flex-col text-center whitespace-nowrap justify-center h-full progress-bar relative  bg-green-600  animate-active" 
                                            style="width: ${parseInt(process.credito/totalCredito*100)}%;"></div>
                                        </div>
                                    </div>
                                </div>
                                <div class=" shrink-0 text-sm ">NIO ${process.credito}</div>
                                
                                <div class="grow">
                                    <div class="relative">
                                        <div
                                            class="w-full  overflow-hidden bg-opacity-10 progress  bg-green-600/25 rounded-full">
                                            <div class="flex flex-col text-center whitespace-nowrap justify-center h-full progress-bar relative  bg-green-600  animate-active" 
                                            style="width: ${parseInt(process.creditoUSD/totalCreditoUSD*100)}%;"></div>
                                        </div>
                                    </div>
                                </div>
                                <div class=" shrink-0 text-sm ">USD ${process.creditoUSD}</div>

                            </div>                            
                        </div>`)}     
                    </main>
                </div>
            </div>
        </div>
        <div class=" grid grid-cols-12 gap-5">            
            <div class="lg:col-span-6 col-span-12">
                <div class="card  custom-class">
                    <header class="card-header  custom-header-class">
                        <div>
                            <div class="card-title custom-class">Egresos USD por transacción</div>
                        </div>
                    </header>
                    <main class="card-body px-5 py-4">
                        ${columnDebitoUSD}
                    </main>
                </div>
            </div>
            <div class="lg:col-span-6 col-span-12">
                <div class="card custom-class">
                    <header class="card-header  custom-header-class">
                        <div>
                            <div class="card-title custom-class">Ingresos USD por transacción</div>
                        </div>
                    </header>
                    <main class="card-body px-5 py-4">
                        ${columnCreditoUSD}
                    </main>
                </div>
            </div>            
        </div>
    </div>`
    }

    CustomStyle = css`
        .component{
           display: block;
        }           
    `
}
customElements.define('w-facturacion-dasboard', FacturacionDasboardView);
export { FacturacionDasboardView }
